...
public class HibernateHtmlFormEntryDAO implements HtmlFormEntryDAO {
	...
	public boolean needsNameAndDescriptionMigration() {
		Query query = sessionFactory.getCurrentSession().createQuery(
		    "select count(*) from HtmlForm where deprecatedName is not null or deprecatedDescription is not null");
	...
	public OpenmrsObject getItemById(Class<? extends OpenmrsObject> type, Integer id) {
		try {
			String idProperty = sessionFactory.getHibernateSessionFactory().getClassMetadata(type)
	...
	public List<Integer> getPersonIdHavingAttributes(String attribute, String attributeValue) {
		String query = "select distinct(pa.person_id) from person_attribute pa, person_attribute_type pat where pa.person_attribute_type_id = pat.person_attribute_type_id and pat.name='"
		        + attribute + "'";
